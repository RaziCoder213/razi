...
export const Dashboard: React.FC = () => {
  ...
  const filterUpcoming = (items: (Movie | TVShow)[], dateRange: string, type: 'movie' | 'series') => {
    ...
    return items.filter(item => {
      const releaseDateStr = type === 'movie' ? (item as Movie).release_date : (item as TVShow).first_air_date;
      ...
      if (releaseTimestamp < todayTimestamp) return false;

      switch (dateRange) {
        case 'today':
          return releaseTimestamp === todayTimestamp;
  ...
  const fetchContent = useCallback(async (currentFilters: FiltersState) => {
    if (!user) return;
    setLoading(true);

    try {
      let moviesData, seriesData;
      const isSearching = currentFilters.searchQuery.trim() !== '';

      if (isSearching) {
        [moviesData, seriesData] = await Promise.all([
          tmdbApi.searchMovies(currentFilters.searchQuery, currentFilters.languages),
          tmdbApi.searchSeries(currentFilters.searchQuery, currentFilters.languages),
        ]);
        
        const upcomingMovies = filterUpcoming(moviesData.results || [], currentFilters.dateRange, 'movie');
        const upcomingSeries = filterUpcoming(seriesData.results || [], currentFilters.dateRange, 'series');

        setMovies(upcomingMovies.slice(0, 12));
        setSeries(upcomingSeries.slice(0, 12));
      } else {
  ...
  }, [user, checkAndSendReleaseNotifications]);
  
  useEffect(() => {
      if (user) {
          fetchContent(filters);
      }
  }, [filters, fetchContent, user]);
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="space-y-8">
            <Card className="bg-card/50 backdrop-blur-sm border-primary/20">
              <CardHeader><CardTitle className="flex items-center text-neon-cyan"><Star className="w-5 h-5 mr-2" />{isSearching ? 'Movie Search Results' : 'Upcoming Movies'}</CardTitle><CardDescription>{isSearching ? `Showing results for "${filters.searchQuery}"` : 'Based on your movie preferences'}</CardDescription></CardHeader>
              <CardContent>
                {loading ? (
                  <div className="text-center text-muted-foreground py-8"><Loader2 className="w-8 h-8 mx-auto mb-2 animate-spin" /><p>Scanning for movies...</p></div>
                ) : movies.length > 0 ? (
                  ...
                  <div className="text-center text-muted-foreground py-8"><Radar className="w-12 h-12 mx-auto mb-2 opacity-50" /><p>No upcoming movies found for your criteria.</p></div>
                )}
              </CardContent>
            </Card>

            <Card className="bg-card/50 backdrop-blur-sm border-primary/20">
              <CardHeader><CardTitle className="flex items-center text-neon-pink"><TrendingUp className="w-5 h-5 mr-2" />{isSearching ? 'Series Search Results' : 'Upcoming Series'}</CardTitle><CardDescription>{isSearching ? `Showing results for "${filters.searchQuery}"` : 'Based on your series preferences'}</CardDescription></CardHeader>
              <CardContent>
                {loading ? (
                  <div className="text-center text-muted-foreground py-8"><Loader2 className="w-8 h-8 mx-auto mb-2 animate-spin" /><p>Scanning for series...</p></div>
                ) : series.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {series.map(show => (
                      <ContentCard 
                  ...
                  <div className="text-center text-muted-foreground py-8"><Radar className="w-12 h-12 mx-auto mb-2 opacity-50" /><p>No upcoming series found for your criteria.</p></div>
                )}
              </CardContent>
            </Card>
          </div>
